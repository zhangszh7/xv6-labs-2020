...
void
kvmunmap(uint64 va, uint64 npages)
{
    uint64 a;
    pte_t *pte;
...
// Given a parent kernel page talbe, copy
// its PTEs into a child's kernel page
// table.
void
kvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
    ...
    for(i = 0; i < sz; i += PGSIZE){
        ...
        kvmmap_proc(new, i, pa, PGSIZE, flags );
    }
}
...
print_level(pagetable_t pagetable, int level)
{
    ...
    for(int i = 0; i < 512; i++){
        ...
        if(level > 0)
            print_level((pagetable_t)pa, level - 1);
    }
    return;
}
